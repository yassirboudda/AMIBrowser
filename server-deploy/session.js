...
// REST endpoints + WebSocket upgrade for screencast streaming + OpenClaw AI
// ============================================================================
...
router.get("/health", (_req, res) => {
  res.json({
    ...
    activeSessions: sm.sessions ? sm.sessions.size : 0,
    maxSessions: sm.MAX_SESSIONS || 2,
    openClawSkills: openClaw.SKILLS.length,
  });
});
...
// ── OpenClaw AI Agent ───────────────────────────────────────────────────────
router.post("/:id/openclaw", requireAuth, requirePremium, async (req, res) => {
  const session = sm.getSession(req.params.id);
  if (!session) return res.status(404).json({ error: "Session not found" });
  if (session.userId !== req.user._id.toString())
    return res.status(403).json({ error: "Not your session" });
  ...
  try {
    const result = await openClaw.processMessage(session, message, history || []);
    // Notify all WebSocket clients about the action
    sm.broadcastToSession(session, {
...
module.exports.handleWebSocketUpgrade = function handleWebSocketUpgrade(
  ...
  wss.on("connection", async (ws, req) => {
    ...
    // Handle incoming messages (input events + OpenClaw commands)
      ...
      try {
        ...
        switch (msg.type) {
          ...
          case "resize":
            ...
            break;
          // ── OpenClaw AI command via WebSocket ───────────────────────────
          case "openclaw": {
            if (!msg.message) {
              ws.send(JSON.stringify({ type: "openclaw_error", message: "message required" }));
              break;
            }
            try {
              ws.send(JSON.stringify({ type: "openclaw_thinking", message: "Processing…" }));
              const result = await openClaw.processMessage(session, msg.message, msg.history || []);
              ...
              // Broadcast navigation changes to all clients
              sm.broadcastToSession(session, {
                type: "navigation",
                url: session.currentUrl,
              });
            } catch (err) {
              ws.send(JSON.stringify({ type: "openclaw_error", message: err.message }));
            }
            break;
          }
          default:
